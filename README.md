...
So far the dashboard should be empty, we haven't run
...
```
puppet_ip=$(docker inspect -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' puppet)

puppet_port=$(docker inspect --format '{{ (index (index .NetworkSettings.Ports "8140/tcp") 0).HostPort }}' puppet)
```

```
$ curl -s -X GET http://$puppet_ip:$puppet_port/pdb/query/v4 --data-urlencode 'query=nodes {}' | jq
{
    "deactivated": null,
